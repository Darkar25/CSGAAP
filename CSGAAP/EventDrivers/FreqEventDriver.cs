...
namespace CSGAAP.EventDrivers
{
    public class FreqEventDriver : NaiveWordEventDriver, INotifyPropertyChanged
    {
        ...
        private static Dictionary<string, ReadOnlyMemory<char>> Frequencies { get; } = Encoding.Default.GetString(Resources.ELPfreq)
        ...
        private IEnumerable<Event> InternalCreateEventSet(ReadOnlyMemory<char> text)
        {
            foreach (var w in base.CreateEventSet(text))
                if (Frequencies.TryGetValue(w.Data.ToString(), out var o))
