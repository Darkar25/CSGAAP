...
{
    public class PorterStemmerEventDriver : NaiveWordEventDriver, INotifyPropertyChanged
    {
        public override string DisplayName => "Word stems";
        public override string ToolTipText => "Word stems derived from the Porter stemmer (English only)";
        public override bool ShowInGUI => API.Instance.Language.Lang == "english";

        public PorterStemmerEventDriver()
        {
        ...
        public override EventSet CreateEventSet(ReadOnlyMemory<char> text) {
            ...
            return new(raw.Select(x =>
            {
                var r = x.ToString();
                var s = r.ToLower();

                // process via Stemmer
                theStemmer.Add(s.ToCharArray(), s.Length);
                theStemmer.Stem();

                var b = theStemmer.ResultBuffer;

                // and hack original case pattern
                for (int ii = 0; ii < theStemmer.ResultLength; ii++)
                    if (char.IsUpper(r[ii]))
        ...
        private class Stemmer
        {
             ...
             * efficient.)
             */
            public override string ToString() => new(b, 0, i_end);

            /**
             * Returns the length of the word resulting from the stemming process.
             */
            public int ResultLength => i_end;
             ...
             */
            public char[] ResultBuffer => b;

            /* cons(i) is true <=> b[i] is a consonant. */

            private bool Cons(int i) => b[i] switch
            {
