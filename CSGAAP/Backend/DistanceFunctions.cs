...
namespace CSGAAP.Backend
{
    public class DistanceFunctions
    {
        ...
        public static DistanceFunction GetDistanceFunction(string action)
        {
            ...
            var driver = List.SingleOrDefault(x => x.DisplayName.Trim().Equals(tmp[0].Trim(), StringComparison.InvariantCultureIgnoreCase)) ?? throw new Exception($"Distance function {tmp[0]} not found!");
